  ...
  <div>
    <div style="max-width: 100%; margin:auto; overflow-y: auto; height: 88vh;">
      <DataTable v-model:editingRows="editingRows" stripedRows scrollable scrollHeight="80vh" sortField="seq"
        ...
        <Column field="_id" sortable header="ID" style="width: 6%">
        </Column>
        ...
        <!-- <Column field="forecast" sortable header="Forecast" style="width: 6%">
          <template #editor="{ data, field }">
            <div class="flex align-items-center">
              <Checkbox v-model="data[field]" inputId="Forecast" name="Forecast" :binary="true" />
              <label for="Forecast" class="ml-2"> Forecast? </label>
            </div>
          </template>
          <template #body="{ data, field }">
        ...
        <Column field="dataFaturado" sortable header="Data Faturamento" style="width: 20%">
          <!-- <template #editor="{ data, field }">
            <span class="p-float-label" style="width: 6vw;">
              <Calendar id="dataFaturado" :inputStyle="{ 'text-align': 'center', 'font-size': '0.8vw ' }"
                v-model="data[field]" :showTime="false" dateFormat="dd/mm/yy" :monthNavigator="true" :yearNavigator="true"
                :showButtonBar="true" autocomplete="on" @date-select="dataAlterada" />
              <label for="dataFaturado"> Data Faturamento </label>
            </span>
          </template> -->
          <template #body="{ data, field, index }">
            {{ cData([data, index]) }}
